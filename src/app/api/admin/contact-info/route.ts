...
import { isContentLocale, type ContactItem } from '@/capabilities/content/content';
import {
  fetchContactInfo,
  replaceContactInfo,
...
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json({ success: true, data: await fetchContactInfo(db, lang) });
  } catch {
    return NextResponse.json(
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    await replaceContactInfo(db, lang, body.items);
