...
import { isContentLocale, type LinkPlatform } from '@/capabilities/content/content';
import {
  fetchLinkPlatforms,
  replaceLinkPlatforms,
...
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json({ success: true, data: await fetchLinkPlatforms(db, lang) });
  } catch {
    return NextResponse.json(
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    await replaceLinkPlatforms(db, lang, body.items);
