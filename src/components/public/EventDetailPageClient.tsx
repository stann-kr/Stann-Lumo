  ...
  return (
    <PageLayout title={event.title} subtitle={`${event.venue}${event.location ? ` · ${event.location}` : ''}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
        {posterPhoto && <div className="order-1 lg:order-2"><img src={getPublicImageUrl(posterPhoto.id)} alt={posterPhoto.altText || event.title} className="w-full object-contain" style={{ objectPosition: `${posterPhoto.focalX}% ${posterPhoto.focalY}%` }} /></div>}
