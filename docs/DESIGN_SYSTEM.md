...
홈 카테고리는 GSAP이 구분선 이동과 내용 등장을 280ms 안에서 함께 제어한다. 구분선은 Flip으로 현재 위치에서 다음 위치로 이동하고, 글자와 이미지는 본래 비율을 유지한다. 연속 선택은 현재 그려진 위치에서 이어지며, 키보드 선택은 즉시 반영한다. 모바일은 강제 스크롤 스냅 없이 콘텐츠 높이로 펼친다. 닫힌 내용은 탐색 대상에서도 제외하며, 움직임 감소 설정과 설정 확인 전에는 정적으로 시작한다.
