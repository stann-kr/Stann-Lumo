...
공개 페이지의 큰 제목은 GSAP으로 글자별 마스크 안에서 엇갈려 들어온다. 홈 패널의 내용과 레드 구획선은 별도 타임라인으로 전개하며, 음악·공연 행과 아카이브 이미지는 문서 스크롤에 맞춰 등장한다. 포인터가 정밀한 기기에서는 링크의 글자·화살표·밑줄과 이미지의 작은 이동·기울기로 호버에 반응한다. 탐색 글자는 롤 전환을 사용하고 헤더의 붉은 선은 읽은 위치를 보여준다.

GSAP은 각 컴포넌트의 범위에서 실행하고, 화면 교체와 움직임 감소 설정 변경 시 해당 애니메이션·이벤트·ScrollTrigger를 정리한다. 키보드로 내용에 진입하면 등장 동작을 즉시 완료한다. 정적 상태에서도 제목과 원본 비율의 이미지, 방문 링크를 읽고 사용할 수 있다. 자동 반복 장식이나 스크롤 고정은 추가하지 않는다.
